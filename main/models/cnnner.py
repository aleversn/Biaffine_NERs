...
class CNNNerv1(BertPreTrainedModel):
    def __init__(self, config: BertConfig):
        ...
        if self.size_embed_dim!=0:
            n_pos = 30
            self.size_embedding = torch.nn.Embedding(n_pos, self.size_embed_dim)
        ...
        if self.n_head>0:
            self.multi_head_biaffine = MultiHeadBiaffine(self.biaffine_size, self.cnn_dim, n_head=self.n_head)
        else:
            self.U = nn.Parameter(torch.randn(self.cnn_dim, self.biaffine_size, self.biaffine_size))
        ...
        if self.cnn_depth>0:
            self.cnn = MaskCNN(self.cnn_dim, self.cnn_dim, kernel_size=self.kernel_size, depth=self.cnn_depth)
    ...
    def decode_labels(self, labels: torch.Tensor, indexes: torch.Tensor):
        ...
        for batch, x, y in zip(*indexes):
            if x<=y and x>=0 and y>=0 and x<length[batch] and y<length[batch]:
                entities[batch].add((x, y, labels[batch, x, y] + 1)) # +1 是由于有O标签
        return entities
    ...
    def decode_logits(self, scores: torch.Tensor, indexes: torch.Tensor, remove_clashed: bool=False, nested: bool=True):
        ...
        # 同labels一样没有特殊的标签
        for batch, x, y in zip(*indexes):
            if x>=0 and x<length[batch] and y>=0 and y<length[batch] and x<=y:
                entities[batch].append((x, y, argmax[batch, x, y] + 1, scores[batch, x, y, argmax[batch, x, y]]))
        for batch in range(len(entities)):
            entities[batch].sort(key=lambda x: x[-1], reverse=True)
        if remove_clashed:
            for batch in range(len(entities)):
                new_entities = []
