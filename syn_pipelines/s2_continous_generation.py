...
parser.add_argument('--file_name', default='cmeee', help='file name of the dataset, you should make sure it contains `train_1000.jsonl` file')
...
for idx, ori_item in tqdm(enumerate(ori_data), total=len(ori_data)):
    text = ''.join(ori_item['text'])
    ori_entities = ["".join(item['text']) + '({})'.format(item['entity']) for item in ori_item["entities"]]
